...
$sql = $cone->pdo->prepare("SELECT * FROM tipoquartos WHERE status = 'Ativo' LIMIT $pagina, $itens_por_pagina");
$sql->execute();
$quarto=$sql->fetch(PDO::FETCH_ASSOC);
$sql_contador = $cone->pdo->prepare("SELECT * FROM tipoquartos WHERE status = 'Ativo'");
...
<head>
	<meta charset="UTF-8" />
	<title>quartos | Sistema de reservas</title>
...
$atlet = $cone->pdo->prepare("SELECT * FROM tipoquartos where status = 'Ativo'");
$atlet->execute();
$sem_quarto = $atlet->fetch(PDO::FETCH_ASSOC);
if(empty($sem_quarto)){
echo '<tr><th>quarto</th><th>numero</th><th>preço</th><th colspan="2"></th></tr>';
echo '<tr>';
echo '<td class="espaco" colspan="6">Não possui quartos cadastrados</td>';
echo '</tr></table><a href="quartos_inativos" class="desativado">quartos Inativos</a>';
}else{
if(!empty($_POST["pesquisa"])){
$busca = $_POST["pesquisa"];
$consulta_quarto = $cone->pdo->prepare("SELECT * FROM tipoquartos WHERE nomequarto LIKE '%$busca%' AND status = 'Ativo' order by nomequarto");
$consulta_quarto->execute();
$vazio = $consulta_quarto->fetch(PDO::FETCH_ASSOC);
if(!empty($vazio)){
$consulta = $cone->pdo->prepare("SELECT * FROM tipoquartos WHERE nomequarto LIKE '%$busca%' AND status = 'Ativo' order by nomequarto");
$consulta->execute();
echo '<tr><th>quarto</th><th>Numero</th><th>preço</th><th colspan="2"></th></tr>';
...
echo'<form action="php/desativa_quarto.php" method="post"><tr>
...
<td><input type="image" src="img/desativar.png" class="desativar" title="Desativar"/></td>
</tr></form>';
}
echo '</table><a href="quartos_inativos" class="desativado">quartos Inativos</a>';
if(empty($_GET["pagina"]))
    $pagina =0;
else
    $pagina = intval($_GET["pagina"]);
$sql1 = $cone->pdo->prepare("SELECT * FROM tipoquartos WHERE nomequarto LIKE '%$busca%' AND status = 'Ativo' limit $pagina, $itens_por_pagina");
$sql1->execute();
$clinte1=$sql1->fetch(PDO::FETCH_ASSOC);
$contador = $cone->pdo->prepare("SELECT * FROM tipoquartos WHERE nomequarto LIKE '%$busca%' AND status = 'Ativo' limit $pagina, $itens_por_pagina");
$contador->execute();
...
<li>
<a href="quartos.php?pagina=0" aria-label="Previous">
...
if($pagina == $i)
$estilo = 'class="active"';
echo'<li '.$estilo.'><a href="quartos.php?pagina='.$i.'">'.($i+1).'</a></li>';
}
echo '<li>
<a href="quartos.php?pagina='.($num_pag-1).'" aria-label="Next">
...
$consulta_quarto = $cone->pdo->prepare("SELECT * FROM tipoquartos where status = 'Ativo' order by nomequarto limit $pagina, $itens_por_pagina");
$consulta_quarto->execute();
echo '<tr><th>quarto</th><th>numero</th><th>Preço</th><th colspan="2"></th></tr>';
...
echo '<li>
<a href="quartos.php?pagina='.($num_paginas-1).'" aria-label="Next">
<span aria-hidden="true">&raquo;</span>
</a>
</li>
</ul>';
