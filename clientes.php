...
<head>
	<meta charset="UTF-8" />
	<title>Clientes | Sistema de reservas</title>
...
<ul>
	<a href="cadastro/pagseguro" title="Check-out"><li> <br>Check-out</li></a>';
...
echo '<tr>';
echo '<td class="espaco" colspan="6">Não possui Clientes cadastrados</td>';
echo '</tr></table><a href="clientes_inativos" class="desativado">Clientes Inativos</a>';
echo ' 
 <p>importar clientes?</p>
  <form method="post" enctype="multipart/form-data" action="php/importar.php">
   <div align="left">  
...
if($pagina == $i)
$estilo = 'class="active"';
echo'<li '.$estilo.'><a href="clientes.php?pagina='.$i.'">'.($i+1).'</a></li>';
}
echo '<li>
<a href="clientes.php?pagina='.($num_pag-1).'" aria-label="Next">
...
$consulta_cliente->execute();
echo '<tr><th>Cliente</th><th>E-mail</th><th>CPF</th><th>Gênero</th><th>Telefone</th><th colspan="4"></th></tr>';
...
echo '</table><a href="" class="desativado"></a>

 <p>IMPORTAR CLIENTES</p>
  <form method="post" enctype="multipart/form-data" action="php/importar.php">
   <div align="left">  
    <label>SELECIONE UMA ARQUIVO CSV:</label>
    <input type="file" name="file" />
    <br />
    <input type="submit" name="submit" value="Importar" class="btn btn-info" />
   </div>
...
echo '<li>
<a href="clientes.php?pagina='.($num_paginas-1).'" aria-label="Next">
<span aria-hidden="true">&raquo;</span>
</a>
</li>
</ul>';
